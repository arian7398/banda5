...
function guardarDatos(datos, sessionToken) {
  try {
    ...
    Logger.log("Token recibido en guardarDatos: " + (sessionToken ? "Presente" : "No proporcionado"));

    // Determinar usuario creador
    let usuarioCreador = 'No especificado';
    ...
    if (sessionToken) {
      try {
        ...
        if (sessionData && sessionData.username) {
          usuarioCreador = sessionData.username;
          Logger.log("Usuario identificado: " + usuarioCreador);
        } else {
          Logger.log("Sesión no válida con token: " + sessionToken);
        }
      } catch (e) {
        Logger.log("Error al validar sesión: " + e.toString());
      }
    }
    
    Logger.log("Guardando registro con usuario: " + usuarioCreador);
    ...
    sheet.appendRow([
      datos.id,
      new Date(), 
      // Guardar nombre de usuario que crea el registro
      usuarioCreador,
...
function actualizarRegistroEnSheet(datos, sessionToken) {
  try {
    // Validar la sesión
    const sessionData = validarSesion(sessionToken);
    if (!sessionData) {
    ...
    for (var i = 0; i < data.length; i++) {
      if (data[i][0] === datos.id) {
        ...
        // Obtener el usuario creador del registro (asumiendo que está en la columna 3)
        // Ajusta este índice si el usuario creador está en otra posición
        usuarioCreador = data[i][2] || '';
        break;
      }
    }
    ...
    if (filaEncontrada > 0) {
      ...
      if (sessionData.role !== 'admin' && sessionData.username !== usuarioCreador) {
        return {
          success: false,
          message: 'No tienes permisos para editar este registro. Solo el creador o un administrador puede modificarlo.'
        };
      }
      
      // Actualizar los valores en la fila encontrada
      // Nota: NO actualizamos el usuario creador (columna 3) para mantener el registro del creador original
      ...
      // Verificar si datosInteresAgraviados está definido antes de usarlo
      if (typeof datosInteresAgraviados !== 'undefined') {
        sheet.getRange(filaEncontrada, 14).setValue(datosInteresAgraviados);
      }
      
      sheet.getRange(filaEncontrada, 15).setValue(funcionEjerce);
      sheet.getRange(filaEncontrada, 16).setValue(tipoEmpresa);
      ...
      // Usar datosInteresDenunciadoTexto en lugar de datos.datosInteresDenunciado
      sheet.getRange(filaEncontrada, 20).setValue(datosInteresDenunciadoTexto);
      ...
      return guardarDatos(datos, sessionToken); // Pasar el token de sesión a guardarDatos
    }
  } catch (error) {
    return {
      success: false,
...
function obtenerRegistros(sessionToken) {
  try {
    ...
    Logger.log("Token recibido en obtenerRegistros: " + (sessionToken ? "Presente" : "No proporcionado"));
    ...
    if (sessionToken) {
      ...
      if (sessionData) {
        ...
        esAdmin = (sessionData.role === 'admin');
        Logger.log("Usuario identificado: " + username + ", Es admin: " + esAdmin);
      } else {
        Logger.log("Sesión no válida o caducada");
        return []; // Si la sesión no es válida, no mostrar registros
      }
    } else {
      Logger.log("No se proporcionó token de sesión");
      return []; // Si no hay token, no mostrar registros
    }
    ...
    for (var i = 1; i < data.length; i++) {
      ...
      // Obtener usuario creador (columna 3)
      const usuarioCreador = row[2] || 'No especificado';
      
      // Filtrar por usuario: solo mostrar registros propios o todos si es admin
      if (esAdmin || usuarioCreador === username || usuarioCreador === 'No especificado') {
        ...
        var registro = {
          id: row[0],
          fechaRegistro: row[1],
          usuarioCreador: usuarioCreador,  // Incluir el usuario creador en los datos
          ...
          // Procesar datos de interés del denunciado (columna 18)
  ...
  try {
    ...
    if (filaEncontrada > 0) {
      ...
      if (sessionData.role !== 'admin' && sessionData.username !== usuarioCreador) {
        return {
          success: false,
          message: 'No tienes permisos para eliminar este registro. Solo el creador o un administrador puede eliminarlo.'
        };
      }
